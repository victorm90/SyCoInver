<x-adminlte-modal id="editModal{{ $item->id }}" title="Editar el Tipo de codigo" size="lg" theme="primary"
    icon="fas fa-bell" v-centered static-backdrop scrollable>
    <div style="height:200px;">
        <p>Ingrese Informacion</p>
        <form action="{{ route('admin.obras.update', $obra) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card">
                <div class="card-body">
                    <x-adminlte-input type="text" name="name" label="Nombre del Codigo" label-class="text-lightblue"
                        value="{{ $item->name }}" AutoSize>
