                ...
                <div class="row">
                    <div class="col-lg">
                        <div class="form-group">
                            <x-adminlte-input type="text" name="name" label="Nombre de la Provincia" disabled="disabled"
                                id="name" placeholder="Texte el Nombre de la Provincia" label-class="text-lightblue"
                                AutoSize>
...
@section('content')
    ...
    <div class="card">
        <div class="header">
            <div class="card-body">
                @php
                    $heads = ['ID', 'Nombre de la Provincia', ['label' => 'Accion', 'no-export' => true, 'width' => 8]];
                    $btnEdit = '';
                    $btnDelete = '';
                ...
                <x-adminlte-datatable id="table1" :heads="$heads" head-theme="dark" :config="$config" striped hoverable
                    ...
                    @foreach ($provincias as $provincia)
                        <tr>
                            ...
                            <td>
                                @if (Auth::user()->roles[0]->name == 'Operador' || Auth::user()->roles[0]->name == 'Admin')
                                    <a href="{{ route('admin.provincias.edit', $provincia) }}"class="btn btn-xs btn-default text-primary mx-1 shadow"
                                        title="Editar"><i class="fa fa-lg fa-fw fa-pen"></i></a>
                                @endif
                                @if (Auth::user()->roles[0]->name == 'Admin')
                                <a href="" class="btn btn-xs btn-default text-danger mx-1 shadow" title="Delete" data-toggle="modal" data-target="#modalPurple{{$provincia->id}}">
                                    <i class="fa fa-lg fa-fw fa-trash"></i></a>
                                @endif
                            </td>
                        </tr>
                        <div class="modal fade" id="modalPurple{{$provincia->id}}" tabindex="-1" role="dialog"
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ¿Está seguro de que desea eliminar la provincia {{ $provincia->name }} ...?
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary"
                                            data-dismiss="modal">Cancelar</button>
                                        <form style="display:inline" action="{{ route('admin.provincias.destroy', $provincia) }}"
                                            method="POST">
                                            @csrf
                                            @method('delete')
