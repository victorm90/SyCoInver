...
@section('content_header')
    ...
    <h1 class="text-center">Listado de Organismos</h1>
    ...
    @if (Auth::user()->roles[0]->name == 'Operador' || Auth::user()->roles[0]->name == 'Admin')
        ...
        "href="{{ route('admin.organismos.create') }}">Agregar Nuevo</a>
    @endif
@stop
                ...
                <x-adminlte-datatable id="table1" :heads="$heads" head-theme="dark" :config="$config" striped hoverable
                    ...
                    @foreach ($organismos as $organismo)
                        <tr>
                            ...
                            <td>
                                @if (Auth::user()->roles[0]->name == 'Operador' || Auth::user()->roles[0]->name == 'Admin')
                                    <a href="{{ route('admin.organismos.edit', $organismo) }}"class="btn btn-xs btn-default text-primary mx-1 shadow"
                                        title="Editar"><i class="fa fa-lg fa-fw fa-pen"></i></a>
                                @endif
                                @if (Auth::user()->roles[0]->name == 'Admin')
                                    <form style="display:inline"
                                        action="{{ route('admin.organismos.destroy', $organismo) }}" method="POST">
                                        @csrf
                                        @method('delete')
                                        {!! $btnDelete !!}
                                    </form>
